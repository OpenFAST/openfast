...
!
!    This file is part of Simple_Cant_Beam.
!
!    Simple_Cant_Beam is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as
!    published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.
!
!    This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty
!    of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more details.
!
!    You should have received a copy of the GNU General Public License along with Simple_Cant_Beam.
!    If not, see <http://www.gnu.org/licenses/>.
!
!**********************************************************************************************************************************
!
!**********************************************************************************************************************************
MODULE Simple_Cant_Beam
   ...
   TYPE(ProgDesc), PARAMETER  :: SCBeam_Ver = ProgDesc( 'Simple_Cant_Beam', 'v1.00.04', '13-February-2013' )

   ! ..... Public Subroutines ...................................................................................................

   PUBLIC :: SCBeam_Init                           ! Initialization routine
   PUBLIC :: SCBeam_End                            ! Ending routine (includes clean up)

   PUBLIC :: SCBeam_UpdateStates                   ! Loose coupling routine for solving for constraint states, integrating
                                                 !   continuous states, and updating discrete states
   PUBLIC :: SCBeam_CalcOutput                     ! Routine for computing outputs

   PUBLIC :: SCBeam_CalcConstrStateResidual        ! Tight coupling routine for returning the constraint state residual
   PUBLIC :: SCBeam_CalcContStateDeriv             ! Tight coupling routine for computing derivatives of continuous states
   PUBLIC :: SCBeam_UpdateDiscState                ! Tight coupling routine for updating discrete states


CONTAINS
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_Init( InitInp, u, p, x, xd, z, OtherState, y, Interval, InitOut, ErrStat, ErrMsg )
!
...
!..................................................................................................................................
      ...
      TYPE(SCBeam_InitInputType),       INTENT(IN   )  :: InitInp     ! Input data for initialization routine
      TYPE(SCBeam_InputType),           INTENT(  OUT)  :: u           ! An initial guess for the input; input mesh must be defined
      TYPE(SCBeam_ParameterType),       INTENT(  OUT)  :: p           ! Parameters
      TYPE(SCBeam_ContinuousStateType), INTENT(  OUT)  :: x           ! Initial continuous states
      TYPE(SCBeam_DiscreteStateType),   INTENT(  OUT)  :: xd          ! Initial discrete states
      TYPE(SCBeam_ConstraintStateType), INTENT(  OUT)  :: z           ! Initial guess of the constraint states
      TYPE(SCBeam_OtherStateType),      INTENT(  OUT)  :: OtherState  ! Initial other/optimization states
      TYPE(SCBeam_OutputType),          INTENT(  OUT)  :: y           ! Initial system outputs (outputs are not calculated;
                                                                      !    only the output mesh is initialized)
      REAL(DbKi),                       INTENT(INOUT)  :: Interval    ! Coupling interval in seconds: the rate that
                                                                      !   (1) SCBeam_UpdateStates() is called in loose coupling &
                                                                      !   (2) SCBeam_UpdateDiscState() is called in tight coupling.
                                                                      !   Input is the suggested time from the glue code;
                                                                      !   Output is the actual coupling interval that will be used
                                                                      !   by the glue code.
      TYPE(SCBeam_InitOutputType),      INTENT(  OUT)  :: InitOut     ! Output for initialization routine
...
!        ErrStat = ErrID_Fatal
!        ErrMsg  = ' Error in Simple_Cant_Beam: Mass must be non-zero to avoid division-by-zero errors.'
!        RETURN
!     END IF
      ...
      IF ( p%method .ne. 1) then
        IF ( p%method .ne. 2) then
          IF ( p%method .ne. 3) then
             ErrStat = ErrID_Fatal
             ErrMsg  = ' Error in Simple_Cant_Beam: integration method must be 1 (RK4), 2 (AB4), or 3 (ABM4)'
             RETURN
          END IF
        END IF
      END IF
      ...
      if ( p%method .eq. 2) then       
         ...
         IF (ErrStat /= 0) THEN
            ErrStat = ErrID_Fatal
            ErrMsg = ' Error in Simple_Cant_Beam: could not allocate OtherStat%xdot.'
            RETURN
         END IF
      ...
      CALL SCBeam_gen_gll(p%order, p%gll_p, p%gll_w, ErrStat, ErrMsg)

      CALL SCBeam_gen_deriv(p%order, p%gll_p, p%gll_deriv, ErrStat, ErrMsg)
...
END SUBROUTINE SCBeam_Init
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_End( u, p, x, xd, z, OtherState, y, ErrStat, ErrMsg )
!
! This routine is called at the end of the simulation.
!..................................................................................................................................

      TYPE(SCBeam_InputType),           INTENT(INOUT)  :: u           ! System inputs
      TYPE(SCBeam_ParameterType),       INTENT(INOUT)  :: p           ! Parameters
      TYPE(SCBeam_ContinuousStateType), INTENT(INOUT)  :: x           ! Continuous states
      TYPE(SCBeam_DiscreteStateType),   INTENT(INOUT)  :: xd          ! Discrete states
      TYPE(SCBeam_ConstraintStateType), INTENT(INOUT)  :: z           ! Constraint states
      TYPE(SCBeam_OtherStateType),      INTENT(INOUT)  :: OtherState  ! Other/optimization states
      TYPE(SCBeam_OutputType),          INTENT(INOUT)  :: y           ! System outputs
      INTEGER(IntKi),                 INTENT(  OUT)  :: ErrStat     ! Error status of the operation
      CHARACTER(*),                   INTENT(  OUT)  :: ErrMsg      ! Error message if ErrStat /= ErrID_None
      ...
      CALL SCBeam_DestroyInput( u, ErrStat, ErrMsg )

      ! Destroy the parameter data:

      CALL SCBeam_DestroyParam( p, ErrStat, ErrMsg )

      ! Destroy the state data:

      CALL SCBeam_DestroyContState(   x,           ErrStat, ErrMsg )
      CALL SCBeam_DestroyDiscState(   xd,          ErrStat, ErrMsg )
      CALL SCBeam_DestroyConstrState( z,           ErrStat, ErrMsg )
      CALL SCBeam_DestroyOtherState(  OtherState,  ErrStat, ErrMsg )

      ! Destroy the output data:

      CALL SCBeam_DestroyOutput( y, ErrStat, ErrMsg )


END SUBROUTINE SCBeam_End
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_UpdateStates( t, n, u, utimes, p, x, xd, z, OtherState, ErrStat, ErrMsg )
!
...
!..................................................................................................................................
      ...
      TYPE(SCBeam_InputType),               INTENT(INOUT) :: u(:)       ! Inputs at utimes
      REAL(DbKi),                           INTENT(IN   ) :: utimes(:)  ! Times associated with u(:), in seconds
      TYPE(SCBeam_ParameterType),           INTENT(IN   ) :: p          ! Parameters
      TYPE(SCBeam_ContinuousStateType),     INTENT(INOUT) :: x          ! Input: Continuous states at t;
                                                                      !   Output: Continuous states at t + Interval
      TYPE(SCBeam_DiscreteStateType),       INTENT(INOUT) :: xd         ! Input: Discrete states at t;
                                                                      !   Output: Discrete states at t  + Interval
      TYPE(SCBeam_ConstraintStateType),     INTENT(INOUT) :: z          ! Input: Initial guess of constraint states at t+dt;
      ...
      TYPE(SCBeam_InputType)            :: u_interp  ! input interpolated from given u at utimes
      TYPE(SCBeam_ContinuousStateType)  :: xdot      ! continuous state time derivative
      ...
      if (p%method .eq. 1) then
         ...
         CALL SCBeam_RK4( t, n, u, utimes, p, x, xd, z, OtherState, ErrStat, ErrMsg )

      else

         ErrStat = ErrID_Fatal
         ErrMsg  = ' Error in SCBeam_UpdateStates: p%method must be 1 (RK4), 2 (AB4), or 3 (ABM4)'
         RETURN
...
END SUBROUTINE SCBeam_UpdateStates
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_CalcOutput( t, u, p, x, xd, z, OtherState, y, ErrStat, ErrMsg )
!
! Routine for computing outputs, used in both loose and tight coupling.
!..................................................................................................................................

      REAL(DbKi),                       INTENT(IN   )  :: t           ! Current simulation time in seconds
      TYPE(SCBeam_InputType),           INTENT(IN   )  :: u           ! Inputs at t
      TYPE(SCBeam_ParameterType),       INTENT(IN   )  :: p           ! Parameters
      TYPE(SCBeam_ContinuousStateType), INTENT(IN   )  :: x           ! Continuous states at t
      TYPE(SCBeam_DiscreteStateType),   INTENT(IN   )  :: xd          ! Discrete states at t
      TYPE(SCBeam_ConstraintStateType), INTENT(IN   )  :: z           ! Constraint states at t
      TYPE(SCBeam_OtherStateType),      INTENT(INOUT)  :: OtherState  ! Other/optimization states
      TYPE(SCBeam_OutputType),          INTENT(INOUT)  :: y           ! Outputs computed at t (Input only so that mesh con-
...
END SUBROUTINE SCBeam_CalcOutput
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_CalcContStateDeriv( t, u, p, x, xd, z, OtherState, xdot, ErrStat, ErrMsg )
!
...
!..................................................................................................................................
      ...
      TYPE(SCBeam_ConstraintStateType), INTENT(IN   )  :: z           ! Constraint states at t
      TYPE(SCBeam_OtherStateType),      INTENT(INOUT)  :: OtherState  ! Other/optimization states
      TYPE(SCBeam_ContinuousStateType), INTENT(INOUT)  :: xdot        ! Continuous state derivatives at t
      INTEGER(IntKi),                   INTENT(  OUT)  :: ErrStat     ! Error status of the operation
      CHARACTER(*),                     INTENT(  OUT)  :: ErrMsg      ! Error message if ErrStat /= ErrID_None
...
END SUBROUTINE SCBeam_CalcContStateDeriv
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_UpdateDiscState( t, n, u, p, x, xd, z, OtherState, ErrStat, ErrMsg )
!
...
END SUBROUTINE SCBeam_UpdateDiscState
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_CalcConstrStateResidual( t, u, p, x, xd, z, OtherState, Z_residual, ErrStat, ErrMsg )
!
...
!..................................................................................................................................
      ...
      TYPE(SCBeam_ContinuousStateType), INTENT(IN   )  :: x           ! Continuous states at t
      TYPE(SCBeam_DiscreteStateType),   INTENT(IN   )  :: xd          ! Discrete states at t
      TYPE(SCBeam_ConstraintStateType), INTENT(IN   )  :: z           ! Constraint states at t (possibly a guess)
      TYPE(SCBeam_OtherStateType),      INTENT(INOUT)  :: OtherState  ! Other/optimization states
      TYPE(SCBeam_ConstraintStateType), INTENT(  OUT)  :: Z_residual  ! Residual of the constraint state equations using
...
END SUBROUTINE SCBeam_CalcConstrStateResidual
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE SCBeam_RK4( t, n, u, utimes, p, x, xd, z, OtherState, ErrStat, ErrMsg )
!
...
!..................................................................................................................................
      ...
      TYPE(SCBeam_InputType),           INTENT(INOUT)  :: u(:)        ! Inputs at t
      REAL(DbKi),                       INTENT(IN   )  :: utimes(:)   ! times of input
      TYPE(SCBeam_ParameterType),       INTENT(IN   )  :: p           ! Parameters
      TYPE(SCBeam_ContinuousStateType), INTENT(INOUT)  :: x           ! Continuous states at t on input at t + dt on output
      TYPE(SCBeam_DiscreteStateType),   INTENT(IN   )  :: xd          ! Discrete states at t
      TYPE(SCBeam_ConstraintStateType), INTENT(IN   )  :: z           ! Constraint states at t (possibly a guess)
      TYPE(SCBeam_OtherStateType),      INTENT(INOUT)  :: OtherState  ! Other/optimization states
      INTEGER(IntKi),                   INTENT(  OUT)  :: ErrStat     ! Error status of the operation
      ...
      TYPE(SCBeam_ContinuousStateType)                 :: xdot        ! time derivatives of continuous states      
      TYPE(SCBeam_ContinuousStateType)                 :: k1          ! RK4 constant; see above
      TYPE(SCBeam_ContinuousStateType)                 :: k2          ! RK4 constant; see above 
      TYPE(SCBeam_ContinuousStateType)                 :: k3          ! RK4 constant; see above 
      TYPE(SCBeam_ContinuousStateType)                 :: k4          ! RK4 constant; see above 
      TYPE(SCBeam_ContinuousStateType)                 :: x_tmp       ! Holds temporary modification to x
      TYPE(SCBeam_InputType)                           :: u_interp    ! interpolated value of inputs 
      ...
      CALL SCBeam_Input_ExtrapInterp( u, utimes, u_interp, t, ErrStat, ErrMsg )

      ! find xdot at t
      CALL SCBeam_CalcContStateDeriv( t, u_interp, p, x, xd, z, OtherState, xdot, ErrStat, ErrMsg )
      ...
      CALL SCBeam_Input_ExtrapInterp(u, utimes, u_interp, t+0.5*p%dt, ErrStat, ErrMsg)

      ! find xdot at t + dt/2
      CALL SCBeam_CalcContStateDeriv( t + 0.5*p%dt, u_interp, p, x_tmp, xd, z, OtherState, xdot, ErrStat, ErrMsg )
      ...
      CALL SCBeam_Input_ExtrapInterp(u, utimes, u_interp, t + p%dt, ErrStat, ErrMsg)

      ! find xdot at t + dt
      CALL SCBeam_CalcContStateDeriv( t + p%dt, u_interp, p, x_tmp, xd, z, OtherState, xdot, ErrStat, ErrMsg )
...
END SUBROUTINE SCBeam_RK4
!----------------------------------------------------------------------------------------------------------------------------------
!----------------------------------------------------------------------------------------------------------------------------------
subroutine SCBeam_gen_gll(N, x, w, ErrStat, ErrMsg)
!
...
!..................................................................................................................................
   ...
   do i = 1, N1
      ...
      if (i==maxit) then
         ErrStat = ErrID_Fatal
         ErrMsg  = ' Error in Simple_Cant_Beam: SCBeam_gen_gll: reached max iterations in gll solve'
      end if
   ...
   return
end subroutine SCBeam_gen_gll
!----------------------------------------------------------------------------------------------------------------------------------
subroutine SCBeam_gen_deriv(N, xgll, deriv, ErrStat, ErrMsg)
!
...
!..................................................................................................................................
   ...
   return
end subroutine SCBeam_gen_deriv
!----------------------------------------------------------------------------------------------------------------------------------
!----------------------------------------------------------------------------------------------------------------------------------
!..................................................................................................................................
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
