   ...
   REAL(ReKi)                      :: RHS_LU(dof_total-12)
   REAL(ReKi)                      :: F_PointLoad(dof_total)
   REAL(ReKi)                      :: feqv(dof_total-6)
   REAL(ReKi)                      :: sol_temp(dof_total-12)
   ...
   DO i=1,niter
       ...
       DO j=1,dof_total-6
           feqv(j) = RHS(j+6)
           RHS_LU(j) = RHS(j+6)
           RHS_LU(j+dof_total) = RHS(j+6+dof_total)
           DO k=1,dof_total-6
               MassM_LU(j,k) = MassM(j+6,k+6)
               MassM_LU(j,k+dof_total) = MassM(j+6,k+dof_total+6)
               MassM_LU(j+dof_total,k) = MassM(j+dof_total+6,k+6)
               MassM_LU(j+dof_total,k+dof_total) = MassM(j+dof_total+6,k+dof_total+6)
           ENDDO
       ENDDO
