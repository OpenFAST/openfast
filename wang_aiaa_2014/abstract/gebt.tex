...
\end{align}
The new matrix notations in Eqs.~\eqref{hatM} to \eqref{hatFext} are briefly
introduced here. $\mathcal{\tens{M}}$ is the sectional mass matrix resolved
in reference configuration; $\mathcal{\vec{F}}^C$ and $\mathcal{\vec{F}}^D$ are
...
\begin{align}
	...
	\end{Bmatrix}
\end{align}
where the following notations were introduced to simplify the above expressions
\begin{align}
    \label{E1}
...
We note that the 3D rotations are represented as Wiener-Milenkovi\'c parameters \cite{Bauchau-etal:2008,Wang:GEBT2013} defined in the following equation:
 \begin{equation}
