...
and $\vec{\hat{u}}^k$ is
the $k^{th}$ nodal value. \textcolor{red}{Note that variable $\xi \in \left[-1,1\right]$ is a non-dimensional quantity defined along the span of the element.}
...
advantages.} In the present implementation, weak-form integrals are evaluated with
