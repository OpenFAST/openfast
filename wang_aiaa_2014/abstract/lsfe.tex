...
\vec{\hat{c}}^k$. It is noted that the composition operator here is an equivalent of $\tens{R}_{\vec{r}} = \tens{R}^T(\vec{p})~\tens{R}(\vec{q}).$
...
the FAST Computer-Aided Engineering (CAE) tool for wind turbine analysis. The system of nonlinear
