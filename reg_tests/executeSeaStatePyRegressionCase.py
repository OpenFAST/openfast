...
"""
    This program executes SeaSate through the python interface for a single test case.
    ...
    Get usage with: `executeSeaStatePyRegressionCase.py -h`
"""
...
parser = argparse.ArgumentParser(description="Executes SeaState c-bindings library interface with a regression test for a single test case.")
parser.add_argument("caseName", metavar="Case-Name", type=str, nargs=1, help="The name of the test case.")
parser.add_argument("executable", metavar="SeaState-Python", type=str, nargs=1, help="The path to the InflowWind driver executable.")
...
moduleDirectory = os.path.join(rtest, "modules", "seastate")
...
if not noExec:
    caseInputFile = os.path.join(testBuildDirectory, "py_seastate_driver.py")
...
localOutFile = os.path.join(testBuildDirectory, "Points.Results.dat")
baselineOutFile = os.path.join(targetOutputDirectory, "Points.Results.dat")
