...
"""
    This program executes Simple-ElastoDyn and a regression test for a single test case.
    ...
    Get usage with: `executeSimpleElastoDynRegressionCase.py -h`
"""
...
parser.add_argument("executable", metavar="SED-Driver", type=str, nargs=1, help="The path to the SED driver executable.")
...
moduleDirectory = os.path.join(rtest, "modules", "simple-elastodyn")
...
if not noExec:
    caseInputFile = os.path.join(testBuildDirectory, "sed_driver.dvr")
    returnCode = openfastDrivers.runSimpleElastodynDriverCase(caseInputFile, executable, verbose=verbose)
    if returnCode != 0:
        rtl.exitWithError("")
    
### Build the filesystem navigation variables for running the regression test
localOutFile = os.path.join(testBuildDirectory, "sed_driver.out")
baselineOutFile = os.path.join(targetOutputDirectory, "sed_driver.out")
