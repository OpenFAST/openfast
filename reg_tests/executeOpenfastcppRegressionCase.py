...
"""
    This program executes OpenFAST through the C++ API and a regression test for a 
    single test case. The test data is contained in a git submodule, r-test, which 
    must be initialized prior to running. See the r-test README or OpenFAST 
    documentation for more info.

    Get usage with: `executeOpenfastcppRegressionCase.py -h`
"""
...
parser = argparse.ArgumentParser(description="Executes OpenFAST-C++ and a regression test for a single test case.")
parser.add_argument("caseName", metavar="Case-Name", type=str, nargs=1, help="The name of the test case.")
parser.add_argument("executable", metavar="OpenFAST-cpp", type=str, nargs=1, help="The path to the OpenFAST-C++ executable.")
...
if not noExec:
    ...
    returnCode = openfastDrivers.runOpenfastcppCase(caseInputFile, executable, testBuildDirectory)
    if returnCode != 0:
        rtl.exitWithError("")
