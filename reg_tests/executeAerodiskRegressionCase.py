...
"""
    This program executes AeroDisk and a regression test for a single test case.
    ...
    Get usage with: `executeAeroDiskRegressionCase.py -h`
"""
...
parser.add_argument("executable", metavar="AeroDisk-Driver", type=str, nargs=1, help="The path to the AeroDisk driver executable.")
...
moduleDirectory = os.path.join(rtest, "modules", "aerodisk")
...
if not noExec:
    caseInputFile = os.path.join(testBuildDirectory, "adsk_driver.dvr")
    returnCode = openfastDrivers.runAerodiskDriverCase(caseInputFile, executable, verbose=verbose)
    if returnCode != 0:
        sys.exit(returnCode*10)
    
### Build the filesystem navigation variables for running the regression test
localOutFile = os.path.join(testBuildDirectory, "adsk_driver.out")
baselineOutFile = os.path.join(targetOutputDirectory, "adsk_driver.out")
