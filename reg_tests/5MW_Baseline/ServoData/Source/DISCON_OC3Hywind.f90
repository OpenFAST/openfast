...
REAL(4), PARAMETER           :: PC_DT         =       0.00125                   ! Communication interval for pitch  controller, sec.
REAL(4), PARAMETER           :: PC_KI         =       0.0008965149   !JASON:REDUCE GAINS FOR HYWIND::0.008068634               ! Integral gain for pitch controller at rated pitch (zero), (-).
...
REAL(4), PARAMETER           :: PC_KP         =       0.006275604    !JASON:REDUCE GAINS FOR HYWIND:0.01882681                ! Proportional gain for pitch controller at rated pitch (zero), sec.
...
REAL(4), PARAMETER           :: VS_DT         =       0.00125                   ! Communication interval for torque controller, sec.
...
IF ( iStatus == 0 )  THEN  ! .TRUE. if we're on the first call to the DLL
   ...
   ErrMsg   = 'Running with torque and pitch control of the NREL offshore '// &
              ...
              'studies.  Includes modifications for the Hywind spar.'   !JASON:CHANGE MESSAGE FOR HYWIND:              'studies.'
   ...
   VS_Slope25 = ( VS_RtPwr/PC_RefSpd           )/( VS_RtGnSp - VS_SySp   ) !JASON:MAKE REGION 3 CONSTANT TORQUE INSTEAD OF CONSTANT POWER FOR HYWIND:   VS_Slope25 = ( VS_RtPwr/VS_RtGnSp           )/( VS_RtGnSp - VS_SySp   )
...
!=======================================================================
   ...
   ! Compute the generator torque, which depends on which region we are in:
      ...
      IF ( (   GenSpeedF >= VS_RtGnSp ) .OR. (  PitCom(1) >= VS_Rgn3MP ) )  THEN ! We are in region 3 - power is constant
         GenTrq = VS_RtPwr/PC_RefSpd   !JASON:MAKE REGION 3 CONSTANT TORQUE INSTEAD OF CONSTANT POWER FOR HYWIND:         GenTrq = VS_RtPwr/GenSpeedF
