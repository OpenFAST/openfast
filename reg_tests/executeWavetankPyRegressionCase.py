...
"""
    This program executes the WaveTank library interface through python interface
    for a single test case. The test data is contained in a git submodule, r-test,
    which must be initialized prior to running. See the r-test README or OpenFAST
    documentation for more info.
    
    Get usage with: `executeWavetankPyRegressionCase.py -h`
"""
...
moduleDirectory = os.path.join(rtest, "glue-codes", "other")
...
if not noExec:
    caseInputFile = os.path.join(testBuildDirectory, "py_wavetank_driver.py")
