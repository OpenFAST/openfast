        ...
        if ( r->type->type_type == DERIVED && ! r->type->usefrom  ) {
          ...
          if ( sw_norealloc_lsh && r->ndims > 0 && has_deferred_dim(r,0) ) {
            ...
            if ( sw == 0 ) {
              ...
              // Create the name of the BINC(C) type for fortran. This depends on we are comminucating a logical,
              // integer, or real to C. 
  ...
  for ( p = ModNames ; p ; p = p->next )
  {
    if ( strlen( p->nickname ) > 0  && ! p->usefrom ) {
      ...
      if ( sw_ccode == 1 ) {
        ...
        //fprintf(fpc,"  #define STDCLL __stdcall\n");    // not needed; ISO_C_BINDING takes care of this
        ...
        //fprintf(fpc,"  #define STDCLL \n");    // not needed; ISO_C_BINDING takes care of this
        fprintf(fpc,"#endif\n\n\n");

        fprintf(fph,"\n#ifdef _WIN32 //define something for Windows (32-bit)\n");
        fprintf(fph,"  #include \"stdbool.h\"\n");
        fprintf(fph,"  #define CALL __declspec( dllexport )\n");
        //fprintf(fph,"  #define STDCLL __stdcall\n");    // not needed; ISO_C_BINDING takes care of this
