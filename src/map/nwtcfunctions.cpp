...
MAPHeaderCallee( )
{
  // Print to the command line to let users know what the environmental conitions are.
  // This will prevent many problem down the road.
  std::cout << "\nMAP " 
            << PROGVERSION 
            << " "
  ...
  try {
    ...
    if ( err.error_status() == MAP_SAFE || err.error_status() == MAP_WARNING ){
      ...
      if ( other.isNumericsUninitialized()==false ){
        //for( int i=0 ; i<other.getSizeOfElement() ; i++)  other.CheckIfElementIsVertical(i);
        errcheck = other.Solve( err, msg );
        //for( int i=0 ; i<other.getSizeOfElement() ; i++) other.ResetVerticalElement(i);
