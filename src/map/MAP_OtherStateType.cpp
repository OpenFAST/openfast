...
{    
  ...
  if ( boost::starts_with( T[1] , "#" ) ) { // if the first elemt in diameter starts with '#'
    ...
    //Msg.RecordToWarningList("Ignoring '#' character preceeding 'Diam' in CableLibrary parameter.");
    std::string str = "";
...
//    PyRun_SimpleString("fig.savefig('/media/sf_Journal_and_Conference_Papers/MAP_Journal/bridle2.pdf', transparent=True)");
...
MAPCALL_GetOutputHeaderUnits( char **arr )
{
  ...
  for ( unsigned int i=0 ; i<element.size() ; i++ ) {
    ...
    if ( this->getElementOptionFlag( i , &Element::LINE_TENSION_flag) ){
      strcpy( arr[count  ], "[N]    " );
      strcpy( arr[count+1], "[N]    " );
      strcpy( arr[count+2], "[N]    " );
      strcpy( arr[count+3], "[N]    " );
      strcpy( arr[count+4], "[N]    " );
      strcpy( arr[count+5], "[N]    " );
      strcpy( arr[count+6], "[N]    " );
      strcpy( arr[count+7], "[N]    " );
      strcpy( arr[count+8], "[N]    " );
      strcpy( arr[count+9], "[N]    " );
      count+=10;      
    }
  }
};
