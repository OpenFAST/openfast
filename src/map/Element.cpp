                 ...
                 MAP_Message                    &msg          ) 
{
  ...
  if (line_property == NULL ){
    ...
    // but so that it can end gracefully. 
...
SetH_and_V_flags( )
{
  ...
  if( this->fairlead->GetNodeType() == Connect ){ // procedure for 'Connect' nodes  
  ...
  } else if( this->fairlead->GetNodeType() == Vessel ) { // procedure for 'Vessel' and 'Fix' nodes        
  ...
  if ( this->anchor->GetNodeType() == Connect ) { // procedure for 'Connect' nodes
...
InitializeCableProperty( const double &g       , 
                         const double &rho_sea ) 
{
  this->A = MAP_CONST::PI*pow( (this->line_property->Diam.value/2) , 2 );  // cross-section area
  this->omega = g*(this->line_property->MassDenInAir.value) - g*A*rho_sea; // weight per length
