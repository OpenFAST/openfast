...
SUBROUTINE SD_GetOP( t, u, p, x, xd, z, OtherState, y, m, ErrStat, ErrMsg, u_op, y_op, x_op, dx_op, xd_op, z_op )
   ...
   IF ( PRESENT( u_op ) ) THEN
      ...
      call PackMotionMesh(u%TPMesh, u_op, idx, FieldMask=FieldMask, UseLogMaps=.true.)
   ...
   IF ( PRESENT( y_op ) ) THEN
      ...
      FieldMask(MASKID_TranslationAcc)  = .true.
      FieldMask(MASKID_RotationAcc)     = .true.
      call PackMotionMesh(y%Y2Mesh, y_op, idx, FieldMask=FieldMask, UseLogMaps=.true.)
      call PackMotionMesh(y%Y3Mesh, y_op, idx, FieldMask=FieldMask, UseLogMaps=.true.)
      idx = idx - 1
      do i=1,p%NumOuts
