...
subroutine InitSettingsFlags( ProgInfo, CLSettings, CLFlags )
   ...
   CLSettings%InputDispFile    =  ""             ! No ADsk input displacement timeseries file name until set
   ...
   CLFlags%InputDispFile       =  .FALSE.        ! No ADsk input displacement timeseries file name until set
...
SUBROUTINE ReadDvrIptFile( DvrFileName, DvrFlags, DvrSettings, ProgInfo, ErrStat, ErrMsg )
   ...
   CHARACTER(1024)                                    :: FileName             ! Name of AeroDisk driver input file
   ...
   IF ( ErrStatTmp /= ErrID_None ) THEN
      CALL SetErrStat(ErrID_Fatal,' Failed to open AeroDisk Driver input file: '//FileName,   &
   ...
   CALL WrScr( 'Opening AeroDisk Driver input file:  '//trim(FileName) )
   ...
   CALL ReadCom( UnIn, FileName,' AeroDisk Driver input file header line 1', ErrStatTmp, ErrMsgTmp )
   if (Failed()) return

   CALL ReadCom( UnIn, FileName, 'AeroDisk Driver input file header line 2', ErrStatTmp, ErrMsgTmp )
   if (Failed()) return

   CALL ReadCom( UnIn, FileName, 'AeroDisk Driver input file seperator line', ErrStatTmp, ErrMsgTmp )
   if (Failed()) return
   ...
   IF ( EchoFileContents ) THEN
      ...
      REWIND(UnIn)
         ...
         ! Reread and echo
      CALL ReadCom( UnIn, FileName,' AeroDisk Driver input file header line 1', ErrStatTmp, ErrMsgTmp, UnEchoLocal )
      if (Failed()) return

      CALL ReadCom( UnIn, FileName, 'AeroDisk Driver input file header line 2', ErrStatTmp, ErrMsgTmp, UnEchoLocal )
      if (Failed()) return

      CALL ReadCom( UnIn, FileName, 'AeroDisk Driver input file seperator line', ErrStatTmp, ErrMsgTmp, UnEchoLocal )
      if (Failed()) return
   ...
   if (Failed()) return
      ...
      ! AeroDisk input file
   CALL ReadVar( UnIn, FileName,DvrSettings%ADskIptFileName,'ADskIptFileName',' AeroDisk input filename',   &
   ...
   !-------------------------------------------------------------------------------------------------
   !  AeroDisk time series input -- this is read from a file of 7 columns (time and 6 dof)
   !-------------------------------------------------------------------------------------------------

      ! InputDispFile input file
   CALL ReadVar( UnIn, FileName,InputChr,'InputDispFile',' AeroDisk input displacements filename',   &
      ErrStatTmp,ErrMsgTmp, UnEchoLocal )
   if (Failed()) return
