...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      IF (SIZE(InData%ld)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%ld))-1 ) = PACK(InData%ld,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%ld)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%ld)>0) OutData%ld = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%ld))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%ld)
    DEALLOCATE(mask1)
  END IF
