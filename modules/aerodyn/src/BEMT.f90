...
#     ifdef UA_OUTS   
         ...
         CALL OpenFOutFile ( UA_OUT_UNIT, 'Debug.UA.out', errStat, errMsg )
               IF (ErrStat >= AbortErrLev) RETURN

   
            ! Heading:
         WRITE (UA_OUT_UNIT,'(/,A)')  'This output information was generated by '//TRIM( GetNVD(BEMT_Ver) )// &
                               ' on '//CurDate()//' at '//CurTime()//'.'
         WRITE (UA_OUT_UNIT,'(:,A20)', ADVANCE='no' ) 'Time'
         do i=1,size(InitOutData_UA%WriteOutputHdr)
            WRITE (UA_OUT_UNIT,'(:,A20)', ADVANCE='no' )  trim(InitOutData_UA%WriteOutputHdr(i))
         end do  
         write (UA_OUT_UNIT,'(A)')    ' '
   
         WRITE (UA_OUT_UNIT,'(:,A20)', ADVANCE='no' ) '(s)'
         do i=1,size(InitOutData_UA%WriteOutputUnt)
            WRITE (UA_OUT_UNIT,'(:,A20)', ADVANCE='no' )  trim(InitOutData_UA%WriteOutputUnt(i))
         end do  
         write (UA_OUT_UNIT,'(A)')    ' '   
#     endif
   else
...
!!#endif
  ...
  ! if ( mod(REAL(t,ReKi),.1) < p%dt) then
   if (allocated(m%y_UA%WriteOutput)) &
            WRITE (UA_OUT_UNIT, '(F20.6,'//trim(num2lstr(size(m%y_UA%WriteOutput)))//'(:,1x,ES19.5E3))') t, ( m%y_UA%WriteOutput(k), k=1,size(m%y_UA%WriteOutput))
  ! end if              
#endif 
