...
! =========  RotInitInputType  =======
  TYPE, PUBLIC :: RotInitInputType
    ...
    REAL(ReKi) , DIMENSION(1:3)  :: NacellePosition      !< X-Y-Z reference position of hub [m]
...
DO i1 = LBOUND(VTK_RotSurfaceTypeData%BladeShape,1), UBOUND(VTK_RotSurfaceTypeData%BladeShape,1)
  CALL AD_Destroyvtk_blsurfacetype( VTK_RotSurfaceTypeData%BladeShape(i1), ErrStat, ErrMsg )
ENDDO
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
      Re_BufSz   = Re_BufSz   + SIZE(InData%NacellePosition)  ! NacellePosition
  ...
  END IF
    DO i1 = LBOUND(InData%NacellePosition,1), UBOUND(InData%NacellePosition,1)
      ReKiBuf(Re_Xferred) = InData%NacellePosition(i1)
      Re_Xferred = Re_Xferred + 1
    END DO
 ...
 ! buffers to store meshes, if any
  ...
  END IF
    ...
    DO i1 = LBOUND(OutData%NacellePosition,1), UBOUND(OutData%NacellePosition,1)
      OutData%NacellePosition(i1) = ReKiBuf(Re_Xferred)
      Re_Xferred = Re_Xferred + 1
    END DO
