      ...
      do k=1,p%rotors(iR)%numBlades
         do j=1,p%rotors(iR)%NumBlNds
            ...
            if (m%FVW%UA_Flag) then
                  ...
                  call UA_CalcOutput(j, k, t, u_UA, m%FVW%p_UA, x%FVW%UA, xd%FVW%UA, OtherState%FVW%UA, p%AFI(p%FVW%AFindx(j,k)), m%FVW%y_UA, m%FVW%m_UA, errStat2, errMsg2 )
                     call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'SetOutputsFromFVW')
...
!..................................................................................................................................
   ...
   do iR=1, size(p%rotors)
      ...
      do IB=1,p%rotors(iR)%numBlades
         ...
         rMax = max(rMax, InitInp%rLocal(1,IB))
         do j=2,p%rotors(iR)%NumBlNds
            ...
            rMax = max(rMax, InitInp%rLocal(j,IB))
         end do !j=nodes
      end do !IB=blades
      ...
      InitInp%UAOff_innerNode = 0
      InitInp%UAOff_outerNode = p%rotors(iR)%NumBlNds + 1
      do IB = 1,p%rotors(iR)%numBlades
         do j = 1,p%rotors(iR)%NumBlNds
            frac = InitInp%rLocal(j,IB) / rMax
            if (frac < InputFileData%UAStartRad) then
               InitInp%UAOff_innerNode(IB) = max(InitInp%UAOff_innerNode(IB), j)
            elseif (frac > InputFileData%UAEndRad) then
               InitInp%UAOff_outerNode(IB) = min(InitInp%UAOff_outerNode(IB), j)
            end if
         end do
      end do      
