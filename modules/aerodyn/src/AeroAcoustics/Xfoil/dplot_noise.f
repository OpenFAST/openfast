...
C
        ...
        CALL PRPLOT_Noise(X0,Y0,TH,UEI,UN,HK,RTHETA,AMSQ,TURB,
     &              -XOFA,-YOFA,FACA,UPRWTS,SA,CA,UDIR,
     &              Delta99,CFNoise)

C       Noise stuff
        Cf(IPR) = TAU(IBLTE(IPR),IPR)/(0.5*QINF**2)
        d99(IPR) =	TH*(3.15 + 1.72/(HK-1.0))+DS	
c        d99(IPR) = Delta99
c        Cf(IPR) = CFNoise
        d_star(IPR) = ds
C
C        WRITE(*,9100) X0,Y0, Cf(IPR), d99(IPR), DS, RTHETA, HK
C 9100   FORMAT(1X,'x y =', 2F8.4,'  Cf =', G12.4,
C     &         '   Delta99 =', G12.4,    
C     &         '   Delta* =', G12.4,
C     &         '   Rtheta =', F10.2,'    Hk =', F9.4)   
C
...
C		
      ...
      SUBROUTINE PRPLOT_Noise(X0,Y0,TH,UE,UN,HK,RET,MSQ,TURB,
