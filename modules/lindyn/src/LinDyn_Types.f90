...
! =========  LD_InitInputType  =======
  TYPE, PUBLIC :: LD_InitInputType
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: x0      !< Degrees of freedom initial conditions [-]
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: xd0      !< Velocities initial conditions [-]
    INTEGER(IntKi) , DIMENSION(:), ALLOCATABLE  :: activeDOFs      !< Degrees of freedom that are active [-]
    character(8)  :: prefix      !< Prefix for degrees of freedom [-]
...
! =========  LD_MiscVarType  =======
  TYPE, PUBLIC :: LD_MiscVarType
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: mMinvC      !< -M^-1 C [-]
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: mMinvK      !< -M^-1 K [-]
...
! =========  LD_InputType  =======
  TYPE, PUBLIC :: LD_InputType
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: Fext      !< External loads [-]
  END TYPE LD_InputType
! =======================
! =========  LD_OutputType  =======
  TYPE, PUBLIC :: LD_OutputType
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: qd 
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: qdd 
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      DO i1 = LBOUND(InData%activeDOFs,1), UBOUND(InData%activeDOFs,1)
        IntKiBuf(Int_Xferred) = InData%activeDOFs(i1)
        Int_Xferred = Int_Xferred + 1
      END DO
  END IF
  ...
  ELSE
    ...
    END IF
      DO i1 = LBOUND(OutData%activeDOFs,1), UBOUND(OutData%activeDOFs,1)
        OutData%activeDOFs(i1) = IntKiBuf(Int_Xferred)
        Int_Xferred = Int_Xferred + 1
      END DO
  END IF
...
! 
   ErrStat = ErrID_None
   ErrMsg  = ""
IF (ALLOCATED(SrcMiscData%mMinvC)) THEN
  i1_l = LBOUND(SrcMiscData%mMinvC,1)
  i1_u = UBOUND(SrcMiscData%mMinvC,1)
  i2_l = LBOUND(SrcMiscData%mMinvC,2)
  i2_u = UBOUND(SrcMiscData%mMinvC,2)
  IF (.NOT. ALLOCATED(DstMiscData%mMinvC)) THEN 
    ALLOCATE(DstMiscData%mMinvC(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%mMinvC.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%mMinvC = SrcMiscData%mMinvC
ENDIF
IF (ALLOCATED(SrcMiscData%mMinvK)) THEN
  i1_l = LBOUND(SrcMiscData%mMinvK,1)
  i1_u = UBOUND(SrcMiscData%mMinvK,1)
  i2_l = LBOUND(SrcMiscData%mMinvK,2)
  i2_u = UBOUND(SrcMiscData%mMinvK,2)
  IF (.NOT. ALLOCATED(DstMiscData%mMinvK)) THEN 
    ALLOCATE(DstMiscData%mMinvK(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%mMinvK.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%mMinvK = SrcMiscData%mMinvK
ENDIF
...
IF (ALLOCATED(MiscData%mMinvC)) THEN
  DEALLOCATE(MiscData%mMinvC)
ENDIF
IF (ALLOCATED(MiscData%mMinvK)) THEN
  DEALLOCATE(MiscData%mMinvK)
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Db_BufSz  = 0
  Int_BufSz  = 0
  Int_BufSz   = Int_BufSz   + 1     ! mMinvC allocated yes/no
  IF ( ALLOCATED(InData%mMinvC) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! mMinvC upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%mMinvC)  ! mMinvC
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! mMinvK allocated yes/no
  IF ( ALLOCATED(InData%mMinvK) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! mMinvK upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%mMinvK)  ! mMinvK
  END IF
  IF ( Re_BufSz  .GT. 0 ) THEN 
     ALLOCATE( ReKiBuf(  Re_BufSz  ), STAT=ErrStat2 )
     IF (ErrStat2 /= 0) THEN 
  ...
  IF ( .NOT. ALLOCATED(InData%mMinvC) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%mMinvC,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%mMinvC,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%mMinvC,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%mMinvC,2)
    Int_Xferred = Int_Xferred + 2

      DO i2 = LBOUND(InData%mMinvC,2), UBOUND(InData%mMinvC,2)
        DO i1 = LBOUND(InData%mMinvC,1), UBOUND(InData%mMinvC,1)
          ReKiBuf(Re_Xferred) = InData%mMinvC(i1,i2)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%mMinvK) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%mMinvK,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%mMinvK,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%mMinvK,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%mMinvK,2)
    Int_Xferred = Int_Xferred + 2

      DO i2 = LBOUND(InData%mMinvK,2), UBOUND(InData%mMinvK,2)
        DO i1 = LBOUND(InData%mMinvK,1), UBOUND(InData%mMinvK,1)
          ReKiBuf(Re_Xferred) = InData%mMinvK(i1,i2)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  Db_Xferred  = 1
  Int_Xferred  = 1
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! mMinvC not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%mMinvC)) DEALLOCATE(OutData%mMinvC)
    ALLOCATE(OutData%mMinvC(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%mMinvC.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i2 = LBOUND(OutData%mMinvC,2), UBOUND(OutData%mMinvC,2)
        DO i1 = LBOUND(OutData%mMinvC,1), UBOUND(OutData%mMinvC,1)
          OutData%mMinvC(i1,i2) = ReKiBuf(Re_Xferred)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! mMinvK not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%mMinvK)) DEALLOCATE(OutData%mMinvK)
    ALLOCATE(OutData%mMinvK(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%mMinvK.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i2 = LBOUND(OutData%mMinvK,2), UBOUND(OutData%mMinvK,2)
        DO i1 = LBOUND(OutData%mMinvK,1), UBOUND(OutData%mMinvK,1)
          OutData%mMinvK(i1,i2) = ReKiBuf(Re_Xferred)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
...
! 
   ErrStat = ErrID_None
   ErrMsg  = ""
IF (ALLOCATED(SrcOutputData%qd)) THEN
  i1_l = LBOUND(SrcOutputData%qd,1)
  i1_u = UBOUND(SrcOutputData%qd,1)
  IF (.NOT. ALLOCATED(DstOutputData%qd)) THEN 
    ALLOCATE(DstOutputData%qd(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOutputData%qd.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstOutputData%qd = SrcOutputData%qd
ENDIF
IF (ALLOCATED(SrcOutputData%qdd)) THEN
  i1_l = LBOUND(SrcOutputData%qdd,1)
  i1_u = UBOUND(SrcOutputData%qdd,1)
  IF (.NOT. ALLOCATED(DstOutputData%qdd)) THEN 
    ALLOCATE(DstOutputData%qdd(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOutputData%qdd.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstOutputData%qdd = SrcOutputData%qdd
ENDIF
...
IF (ALLOCATED(OutputData%qd)) THEN
  DEALLOCATE(OutputData%qd)
ENDIF
IF (ALLOCATED(OutputData%qdd)) THEN
  DEALLOCATE(OutputData%qdd)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  IF ( ALLOCATED(InData%qd) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! qd upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%qd)  ! qd
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! qdd allocated yes/no
  IF ( ALLOCATED(InData%qdd) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! qdd upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%qdd)  ! qdd
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      DO i1 = LBOUND(InData%qd,1), UBOUND(InData%qd,1)
        ReKiBuf(Re_Xferred) = InData%qd(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%qdd) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%qdd,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%qdd,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%qdd,1), UBOUND(InData%qdd,1)
        ReKiBuf(Re_Xferred) = InData%qdd(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    ALLOCATE(OutData%qd(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
      DO i1 = LBOUND(OutData%qd,1), UBOUND(OutData%qd,1)
        OutData%qd(i1) = ReKiBuf(Re_Xferred)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! qdd not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%qdd)) DEALLOCATE(OutData%qdd)
    ALLOCATE(OutData%qdd(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%qdd.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i1 = LBOUND(OutData%qdd,1), UBOUND(OutData%qdd,1)
        OutData%qdd(i1) = ReKiBuf(Re_Xferred)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
...
!..................................................................................................................................
 ...
 t_out = tin_out - tin(1)
   ...
   ScaleFactor = t_out / t(2)
IF (ALLOCATED(y_out%qd) .AND. ALLOCATED(y1%qd)) THEN
  DO i1 = LBOUND(y_out%qd,1),UBOUND(y_out%qd,1)
    b = -(y1%qd(i1) - y2%qd(i1))
    y_out%qd(i1) = y1%qd(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
IF (ALLOCATED(y_out%qdd) .AND. ALLOCATED(y1%qdd)) THEN
  DO i1 = LBOUND(y_out%qdd,1),UBOUND(y_out%qdd,1)
    b = -(y1%qdd(i1) - y2%qdd(i1))
    y_out%qdd(i1) = y1%qdd(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
...
IF (ALLOCATED(y_out%qd) .AND. ALLOCATED(y1%qd)) THEN
  DO i1 = LBOUND(y_out%qd,1),UBOUND(y_out%qd,1)
    b = (t(3)**2*(y1%qd(i1) - y2%qd(i1)) + t(2)**2*(-y1%qd(i1) + y3%qd(i1)))* scaleFactor
    c = ( (t(2)-t(3))*y1%qd(i1) + t(3)*y2%qd(i1) - t(2)*y3%qd(i1) ) * scaleFactor
    y_out%qd(i1) = y1%qd(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ALLOCATED(y_out%qdd) .AND. ALLOCATED(y1%qdd)) THEN
  DO i1 = LBOUND(y_out%qdd,1),UBOUND(y_out%qdd,1)
    b = (t(3)**2*(y1%qdd(i1) - y2%qdd(i1)) + t(2)**2*(-y1%qdd(i1) + y3%qdd(i1)))* scaleFactor
    c = ( (t(2)-t(3))*y1%qdd(i1) + t(3)*y2%qdd(i1) - t(2)*y3%qdd(i1) ) * scaleFactor
    y_out%qdd(i1) = y1%qdd(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ALLOCATED(y_out%WriteOutput) .AND. ALLOCATED(y1%WriteOutput)) THEN
  DO i1 = LBOUND(y_out%WriteOutput,1),UBOUND(y_out%WriteOutput,1)
