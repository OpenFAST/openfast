...
subroutine SlD_ReadInput( InputFileName, EchoFileName, InputFileData, ErrStat, ErrMsg )
   ...
   character(35)                                      :: Fmt                  !< Output format for logical parameters. (matches NWTC Subroutine Library format)
   ...
   call ReadCom( UnitInput, InputFileName, 'SoilDyn input file separator line',  TmpErrStat, TmpErrMsg, UnitEcho );   if (Failed()) return;
      ...
      ! In general, the stiffness and damping matrices will have the following symmetries:
   !  K11 = K22
   !  K15 = -K24
   !  K51 = -K42
   !  K55 = K44
...
!! routine checks all the parameters that are common with all the wind types, then calls subroutines
...
SUBROUTINE SlD_OpenSumFile( SumFileUnit, SummaryName, IfW_Prog, WindType, ErrStat, ErrMsg )
   INTEGER(IntKi),                  INTENT(  OUT)  :: SumFileUnit    !< the unit number for the SoilDyn summary file
...
SUBROUTINE SlD_CloseSumFile( SumFileUnit, ErrStat, ErrMsg )
   INTEGER(IntKi),                  INTENT(INOUT)  :: SumFileUnit    !< the unit number for the SoilDyn summary file
   INTEGER(IntKi),                  INTENT(  OUT)  :: ErrStat        !< returns a non-zero value when an error occurs
   CHARACTER(*),                    INTENT(  OUT)  :: ErrMsg         !< Error message if ErrStat /= ErrID_None
