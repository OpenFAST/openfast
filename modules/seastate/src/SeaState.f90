...
SUBROUTINE SeaSt_CalcOutput( Time, u, p, x, xd, z, OtherState, y, m, ErrStat, ErrMsg )   
   ...
   IF ( p%NumOuts > 0 ) THEN
      ...
      DO i = 1, p%NWaveKin
         ...
         CALL WaveField_GetNodeWaveKin( p%WaveField, Time, positionXYZ, .FALSE., .TRUE., nodeInWater, zeta1, zeta2, zeta, WaveDynP(i), WaveVel(:,i), WaveAcc(:,i), WaveAccMCF(:,i), ErrStat2, ErrMsg2 )
           CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
      END DO
