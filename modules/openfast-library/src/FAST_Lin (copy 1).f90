...
SUBROUTINE FAST_Linearize_OP(t_global, p_FAST, y_FAST, m_FAST, ED, BD, SrvD, AD14, AD, IfW, OpFM, HD, SD, MAPp, FEAM, MD, Orca, &
   ...
   if ( p_FAST%CompSub  == Module_ExtPtfm ) then 
      ...
      call ExtPtfm_JacobianPInput( t_global, ExtPtfm%Input(1), ExtPtfm%p, ExtPtfm%x(STATE_CURR), ExtPtfm%xd(STATE_CURR), ExtPtfm%z(STATE_CURR), ExtPtfm%OtherSt(STATE_CURR), &
                                 ExtPtfm%Output(1), ExtPtfm%m, ErrStat2, ErrMsg2, dYdu=y_FAST%Lin%Modules(Module_ExtPtfm)%Instance(1)%D, dXdu=y_FAST%Lin%Modules(Module_ExtPtfm)%Instance(1)%B )
      !if(Failed()) return;
      !   
      ...
      !                               ExtPtfm%Output(1), ExtPtfm%m, ErrStat2, ErrMsg2, dYdx=y_FAST%Lin%Modules(Module_ExtPtfm)%Instance(1)%C, dXdx=y_FAST%Lin%Modules(Module_ExtPtfm)%Instance(1)%A )
      !if(Failed()) return;
      !
      !   ! get the operating point
      ...
      !                  ExtPtfm%Output(1), ExtPtfm%m, ErrStat2, ErrMsg2, u_op=y_FAST%Lin%Modules(Module_ExtPtfm)%Instance(1)%op_u, y_op=y_FAST%Lin%Modules(Module_ExtPtfm)%Instance(1)%op_y, &
