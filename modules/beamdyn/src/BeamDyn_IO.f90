...
SUBROUTINE BD_PrintSum( p, x, m, InitInp, ErrStat, ErrMsg )
   ...
   call yaml_write_var  (UnSu, 'Bld_mass'    , p%blade_mass    , 'F13.3', ErrStat, ErrMsg, comment='(kg)')
   call yaml_write_var  (UnSu, 'Bld_length'  , p%blade_length  , 'F13.3', ErrStat, ErrMsg, comment='(m)')
   call yaml_write_list (UnSu, 'Bld_CG'      , p%blade_CG      , 'ES18.5', ErrStat, ErrMsg, comment='Blade center of mass (IEC coords) (m)')
   call yaml_write_array(UnSu, 'Bld_IN'      , p%blade_IN      , 'ES18.5', ErrStat, ErrMsg, comment='Blade mass moment of inertia')
   call yaml_write_list (UnSu, 'GlbPos'      , p%GlbPos        , 'ES18.5', ErrStat, ErrMsg, comment='Global position vector (IEC coords)')
   call yaml_write_array(UnSu, 'GlbRot'      , p%GlbRot        , 'ES18.5', ErrStat, ErrMsg, comment='Global rotation tensor (IEC coords)')
   call yaml_write_array(UnSu, 'BldRootOri'  , InitInp%RootOri , 'ES18.5', ErrStat, ErrMsg, comment='Initial blade orientation tensor (relative to global rotation tensor)')
   call yaml_write_list (UnSu, 'Glb_crv'     , p%Glb_crv       , 'ES18.5', ErrStat, ErrMsg, comment='Global rotation WM parameters (IEC coords)')
...
!FIXME:analysis_type
   ...
   WRITE (UnSu,'(A)')  '# Initial position and Weiner-Milenkovic rotation vectors (IEC coordinate system)'
   DO i=1,p%elem_total
       WRITE (UnSu, '("#", 1x,A,I4)') 'Element number: ',i
       WRITE (UnSu, '("#", 6(2x,A))') '            X        ','        Y        ','        Z        ','      WM_x       ','      WM_y       ','      WM_z       '
       WRITE (UnSu, '("#", 6(2x,A))') '    -----------------','-----------------','-----------------','-----------------','-----------------','-----------------'
       call yaml_write_array(UnSu, 'Init_Node'//num2lstr(i), transpose(p%uuN0(1:6,:,i)), p%OutFmt, ErrStat, ErrMsg, AllFmt='6(ES18.5,",")')
   ENDDO

   WRITE (UnSu,'(/,A)')  '# Quadrature point position and rotation vectors'
   DO i=1,p%elem_total
       WRITE (UnSu, '("#", 1x,A,I4)')  'Element number: ',i
       WRITE (UnSu, '("#", 6(2x,A))') '            X        ','        Y        ','        Z        ','      WM_x       ','      WM_y       ','      WM_z       '
       WRITE (UnSu, '("#", 6(2x,A))') '    -----------------','-----------------','-----------------','-----------------','-----------------','-----------------'
       call yaml_write_array(UnSu, 'Init_QP'//num2lstr(i), transpose(p%uu0(1:6,:,i)), p%OutFmt, ErrStat, ErrMsg, AllFmt='6(ES18.5,",")')
   ENDDO
