module FVW_BiotSavart 
   ...
   integer(IntKi), parameter :: idRegCompact    = 4
   integer(IntKi), parameter :: idRegCompactDim = 5
   integer(IntKi), parameter, dimension(5) :: idRegVALID      = (/idRegNone,idRegRankine,idRegLambOseen,idRegVatistas,idRegCompact/)

contains


!> Induced velocity from one segment at one control points
subroutine ui_seg_11(DeltaPa, DeltaPb, Gam, RegModel , RegParam, Ui)
   implicit none 
   ! Input/output arguments 
   real(ReKi), dimension(3), intent(in) :: DeltaPa    !< 3 x 1   Pcp-P1 
   real(ReKi), dimension(3), intent(in) :: DeltaPb    !< 3 x 1   Pcp-P2
   real(ReKi), intent(in)               :: Gam        !< Circulation
   integer, intent(in)                  :: RegModel   !< Regularization model
   real(ReKi), intent(in)               :: RegParam   !< Regularization parameter
   real(ReKi), dimension(3),intent(out) :: Ui         !< Induced velocity (no side effects)
   ...
   elseif(abs(xb)+abs(yb)+abs(zb)<MINNORMSIMP) then
      ...
      if (norm2_r0<PRECISION_UI .or. denominator<PRECISION_UI .or. norm2_crossprod<PRECISION_UI) then
         ...
         !--- Normal Procedure 
         ! smooth model  
         select case (RegModel) !
         case ( idRegNone ) !! No vortex core model
            Kv=1.0_ReKi
         case ( idRegRankine ) !!Rankine - t<=>rc
            ! orthogonal distance r1xr2/r0 
            h2 = norm2_crossprod/ norm2_r0 
            RegParam2=RegParam**2
            if (h2<RegParam2) then 
               Kv=h2/RegParam2
            else
               Kv=1.0_ReKi 
            end if 
         case ( idRegLambOseen ) !!Lamb-Oseen - vsic_param<=>rc 
            ! orthogonal distance r1xr2/r0 
            h2 = norm2_crossprod/ norm2_r0 
            RegParam2=RegParam**2
            exp_value = -1.25643_ReKi*(h2)/(RegParam2)
            if(exp_value<MIN_EXP_VALUE) then
               Kv = 1.0_ReKi
            else
               Kv = 1.0_ReKi-exp(exp_value)
            endif
         case ( idRegVatistas ) !!Vatistas n=2 - RegParam<=>rc 
            ! orthogonal distance r1xr2/r0 
            h2 = norm2_crossprod/ norm2_r0 
            RegParam2=RegParam**2
            ! h = (norm_a+norm_b)/2; 
            Kv = h2/sqrt(RegParam2**2+h2**2)
         case ( idRegCompact ) !!Cut-off radius as function of length: delta^2*norm(r0)^2  
            h2 = norm2_crossprod/ norm2_r0 
            Kv=1.0_ReKi
            ! delta*norm(r0)^2  
            denominator=denominator+RegParam*norm2_r0
            RegParam2=RegParam**2 ! TODO
         case ( idRegCompactDim ) !!Cut-off radius dimension fixed: (delta l_0)^2  
            h2 = norm2_crossprod/ norm2_r0 
            Kv=1.0_ReKi
            ! (delta l_0)^2  
            denominator=denominator+RegParam
            RegParam2=RegParam**2 ! TODO
         case ( 33 ) !!Vatistas n=2 - RegParamt<=>rc  
...
subroutine ui_seg(iCPStart, iCPEnd, nCPsTot, CPs, &
   ...
   integer(IntKi), intent(in)                       :: RegModel       !< Regularization model 
      ...
      do is=iSegStart,iSegEnd ! loop on selected segments 
         ...
         elseif(abs(xb)+abs(yb)+abs(zb)<MINNORMSIMP) then
            ...
            if (denominator<PRECISION_UI .or. norm2_crossprod<PRECISION_UI) then
               ...
               case ( idRegCompact ) !!Cut-off radius as function of length: delta^2*norm(r0)^2  
                  Kv=1.0_ReKi
                  denominator=denominator+RegParam1*RegParam1*norm2_r0
               case ( idRegCompactDim ) !!Cut-off radius dimension fixed: (delta l_0)^2  
                  Kv=1.0_ReKi
                  denominator=denominator+RegParam1*RegParam1
               case ( 33 ) !!Vatistas n=2 - RegParamt<=>rc  
                  ! See Hoydonck 2012
                  h           = (norm_a+norm_b)/2.0_ReKi
                  RegParam2 = RegParam1**2
...
subroutine ui_quad_n1(CPs, nCPs, P1, P2, P3, P4, Gamm, RegModel, RegParam, Uind)
   ...
   do icp=1,nCPs
      ...
      ! 1-2 segment
      DP1=CP-P1; DP2=CP-P2; 
      call ui_seg_11 ( DP1, DP2, Gamm, RegModel, RegParam, Uindtmp)
