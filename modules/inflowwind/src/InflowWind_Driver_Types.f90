...
subroutine InflowWind_Driver_CopyIfWDriver_Settings(SrcIfWDriver_SettingsData, DstIfWDriver_SettingsData, CtrlCode, ErrStat, ErrMsg)
   ...
   if (allocated(SrcIfWDriver_SettingsData%OutWindZ)) then
      LB(1:1) = lbound(SrcIfWDriver_SettingsData%OutWindZ, kind=B8Ki)
      UB(1:1) = ubound(SrcIfWDriver_SettingsData%OutWindZ, kind=B8Ki)
   ...
   if (allocated(SrcIfWDriver_SettingsData%OutWindY)) then
      LB(1:1) = lbound(SrcIfWDriver_SettingsData%OutWindY, kind=B8Ki)
      UB(1:1) = ubound(SrcIfWDriver_SettingsData%OutWindY, kind=B8Ki)
   ...
   if (allocated(SrcIfWDriver_SettingsData%OutWindX)) then
      LB(1:1) = lbound(SrcIfWDriver_SettingsData%OutWindX, kind=B8Ki)
      UB(1:1) = ubound(SrcIfWDriver_SettingsData%OutWindX, kind=B8Ki)
