...
!----------------------------------------------------------------------------------------------------
  ...
  CurLine = CurLine + 1  ! Skip section break
      ...
      ! LIDAR Sensor Type
   CALL ParseVar( InFileInfo, CurLine, "SensorType", InputFileData%SensorType, TmpErrStat, TmpErrMsg )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   IF (ErrStat >= AbortErrLev) RETURN

      ! Number of Range Gates
   CALL ParseVar( InFileInfo, CurLine, "NumPulseGate", InputFileData%NumPulseGate, TmpErrStat, TmpErrMsg )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   IF (ErrStat >= AbortErrLev) RETURN

        
      ! Rotor Apex Offset Position
   CALL ParseAry( InFileInfo, CurLine, "RotorApexOffsetPos", InputFileData%RotorApexOffsetPos, 1, TmpErrStat, TmpErrMsg )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   IF (ErrStat >= AbortErrLev) RETURN

      ! LIDAR Radial V
   CALL ParseLoVar( InFileInfo, CurLine, "LidRadialVel", InputFileData%LidRadialVel, TmpErrStat, TmpErrMsg )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   IF (ErrStat >= AbortErrLev) RETURN 
   
    CALL ParseVar( InFileInfo, CurLine, "FocalDistanceX", InputFileData%FocalDistanceX, TmpErrStat, TmpErrMsg, UnEc )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   if (Failed()) return
   
   CALL ParseVar( InFileInfo, CurLine, "FocalDistanceY", InputFileData%FocalDistanceY, TmpErrStat, TmpErrMsg, UnEc )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   if (Failed()) return
   
     CALL ParseVar( InFileInfo, CurLine, "FocalDistanceZ", InputFileData%FocalDistanceZ, TmpErrStat, TmpErrMsg, UnEc )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   if (Failed()) return
   

       ! MAXDLLChainOutputs
   CALL ParseVar( InFileInfo, CurLine, "MAXDLLChainOutputs", InputFileData%MAXDLLChainOutputs, TmpErrStat, TmpErrMsg )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   IF (ErrStat >= AbortErrLev) RETURN
   
     ! Measurement Interval
   CALL ParseVar( InFileInfo, CurLine, "t_measurement_interval", InputFileData%t_measurement_interval, TmpErrStat, TmpErrMsg )
   CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   IF (ErrStat >= AbortErrLev) RETURN
...
SUBROUTINE InflowWind_ValidateInput( InitInp, InputFileData, ErrStat, ErrMsg )
   ...
   if (InitInp%Linearize) then
      ...
      if (InputFileData%SensorType /= -1) then
