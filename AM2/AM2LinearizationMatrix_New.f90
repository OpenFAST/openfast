...
!   temp_B(:,:) = temp_B(:,:) - temp_B0(:,:) + temp_H0(:,:)
   ...
   A2(4:6,4:6) = MATMUL(Tilde(vel),Tilde(mEta)) + &
                &MATMUL(MATMUL(rho,Tilde(omega)),temp_H) - &
                &MATMUL(Tilde(MATMUL(rho,omega)),temp_H)

   A3(:,:) = 0.0D0
   A3(1:3,4:6) = MATMUL(Tilde(omega0),TRANSPOSE(Tilde(mEta)))
   A3(4:6,4:6) = MATMUL(Tilde(vel0),Tilde(mEta)) + &
                &MATMUL(MATMUL(rho,Tilde(omega0)),temp_H) - &
                &MATMUL(Tilde(MATMUL(rho,omega0)),temp_H)

   A1(:,:) = 0.0D0
   A1(1:3,4:6) = MATMUL(Tilde(omed0),TRANSPOSE(Tilde(mEta)))
   A1(4:6,4:6) = MATMUL(Tilde(acc0),Tilde(mEta)) + &
                &MATMUL(MATMUL(rho,Tilde(omed0)),temp_H) - &
                &MATMUL(Tilde(MATMUL(rho,omed0)),temp_H)
   ...
   A5(4:6,4:6) = MATMUL(MATMUL(Tilde(omega),MATMUL(rho,Tilde(omega))),temp_H) - &
                &MATMUL(MATMUL(Tilde(omega),Tilde(MATMUL(rho,omega))),temp_H)
