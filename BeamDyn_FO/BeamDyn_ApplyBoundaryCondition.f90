   ...
   x%q(1:3) = u%PointMesh%TranslationDisp(1:3,1)

   temp_R(:,:) = 0.0D0
   temp_rot(:) = 0.0D0
   temp_R(1:3,1:3) = u%PointMesh%Orientation(1:3,1:3,1)
   CALL CrvExtractCrv(temp_R,temp_rot)
   x%q(4:6) = temp_rot(1:3)

   x%dqdt(1:3) = u%PointMesh%TranslationVel(1:3,1)
   x%dqdt(4:6) = u%PointMesh%RotationVel(1:3,1)
