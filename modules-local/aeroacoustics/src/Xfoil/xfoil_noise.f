...
C===============================================
c      IF(ISNACA) THEN
c       READ(airfoil,*) IINPUT
c       CALL NACA(IINPUT)
cC===============================================
c      ELSE
c       CALL LOAD(airfoil,ITYPE)
c       IF(ITYPE.GT.0 .AND. NB.GT.0) THEN
ccc       CALL PANGEN(.TRUE.)
c        CALL ABCOPY(.TRUE.)
C
c        CALL CANG(X,Y,N,0, IMAX,AMAX)
c        IF(ABS(AMAX).GT.ANGTOL) THEN
c         WRITE(*,1081) AMAX, IMAX
c         CALL PANPLT
c        ENDIF
c       ENDIF
c      ENDIF
...
C
      ...
      !CALL AREAD(FNAME,IBX,XB,YB,NB,NAME,ISPARS,ITYPE,1)
	NB=NB_AFMODULE
	XB(1:NB)=XB_AFMODULE
	YB(1:NB)=YB_AFMODULE
	ITYPE=2
	NAME='s809.dat' !notused
