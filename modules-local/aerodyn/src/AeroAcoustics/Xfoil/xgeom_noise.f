...
C
   ...
   20 CONTINUE
C      WRITE(*,*) 'LEFIND:  LE point not found.  Continuing...'
      SLE = S(I)
      RETURN
      END
...
C
 ...
 300  CONTINUE
C   WRITE(*,*)
C     &      'SOPPS: Opposite-point location failed. Continuing...'
 303    SOPP = SLE + SFRAC*(S(INOPP)-SLE)
C
 305  CONTINUE
      RETURN
      END ! SOPPS
...
C
   ...
   10 CONTINUE
C      WRITE(*,*) 'SSS: failed to converge subtending angle points'
      S1 = SS
      S2 = SS
C
   11 CONTINUE
...
C        WRITE(*,1100) I, X(I), Y(I), S(I), ADEG, CV
   ...
   30 CONTINUE
C
C      WRITE(*,1200) CMAX, IMAX, X(IMAX), Y(IMAX), S(IMAX)
C
      RETURN
C
C 1050 FORMAT(/'  i       x        y        s       theta        curv')
CCC             120   0.2134  -0.0234  -0.0234   180.024      2025.322
C 1100 FORMAT(1X,I3, 3F9.4, F11.3, F12.3)
C 1200 FORMAT(/' Maximum curvature =', F14.3,
C     &        '   at  i,x,y,s  = ', I3, 3F9.4 )
      END ! CLIS
