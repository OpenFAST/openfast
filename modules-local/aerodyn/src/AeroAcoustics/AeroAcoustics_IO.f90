   ...
   CALL GetPath( InputFile, PriPath )     ! Input files will be relative to the path where the primary input file is located.
	...
	FileName = TRIM(PriPath)//'TIGrid_In.txt'
     ...
     CALL OpenFInpFile ( UnIn, FileName, ErrStat2, ErrMsg2 )
      ...
      IF ( ErrStat2 >= AbortErrLev ) THEN
         print*, 'File Not Found ', FileName
         print*, 'Aborting due to missing file AeroAcoustics require for TI Grid because interpolation is on in Aeroacoustics.dat '
 	 call abort
         RETURN
      END IF


      CALL ReadCom( UnIn, FileName, 'Text Line', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )   
      CALL ReadVar( UnIn, FileName, GridY, 'GridY',   'Echo flag', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
		 
      CALL ReadCom( UnIn, FileName, 'Text Line', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )   
      CALL ReadVar( UnIn, FileName, GridZ, 'GridZ',   'Echo flag', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )

      CALL ReadCom( UnIn, FileName, 'Text Line', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )   
      CALL ReadVar( UnIn, FileName, InputFileData%dy_turb_in, 'InputFileData%dy_turb_in',   'Echo flag', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )

      CALL ReadCom( UnIn, FileName, 'Text Line', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )   
      CALL ReadVar( UnIn, FileName, InputFileData%dz_turb_in, 'InputFileData%dz_turb_in',   'Echo flag', ErrStat2, ErrMsg2, UnEc )
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
		 
      CALL AllocAry( InputFileData%TI_Grid_In,GridZ,GridY,'InputFileData%TI_Grid_In', ErrStat2, ErrMsg2)
         CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
		 print*, size(InputFileData%TI_Grid_In,1),size(InputFileData%TI_Grid_In,2)
