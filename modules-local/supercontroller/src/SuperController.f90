...
module SuperController
   ...
   abstract interface
      subroutine SC_DLL_GetInitData_PROC (nTurbines, NumParamGlobal, NumParamTurbine, ParamGlobal, ParamTurbine, NumSC2CtrlGlob, from_SCglob, NumSC2Ctrl, from_SC, errStat, errMsg )  BIND(C)
         use, intrinsic :: ISO_C_Binding
         integer(C_INT),            intent(in   ) :: nTurbines         !< number of turbines connected to this supercontroller
...
#ifdef STATIC_DLL_LOAD   
   interface
      subroutine SC_DLL_GetInitData ( nTurbines, NumParamGlobal, NumParamTurbine, ParamGlobal, ParamTurbine, NumSC2CtrlGlob, from_SCglob, NumSC2Ctrl, from_SC, errStat, errMsg )  BIND(C)
         use, intrinsic :: ISO_C_Binding
         integer(C_INT),            intent(in   ) :: nTurbines         !< number of turbines connected to this supercontroller
         integer(C_INT),            intent(in   ) :: NumParamGlobal      !< number of global parameters
         integer(C_INT),            intent(in   ) :: NumParamTurbine     !< number of parameters per turbine
   ...
   subroutine SC_Init(InitInp, u, p, x, xd, z, OtherState, y, m, interval, InitOut, errStat, errMsg )
      ...
      if ( (p%NumParamGlobal > 0) .or. (p%NumParamTurbine > 0) .or. (p%NumSC2CtrlGlob > 0) .or. (p%NumSC2Ctrl > 0) ) then 
         ...
         if ( p%NumParamTurbine > 0 ) then
            ...
            ! if we're statically loading the library (i.e., OpenFOAM), we can just call SC_INIT(); 
         call SC_DLL_GetInitData( p%nTurbines, p%NumParamGlobal, p%NumParamTurbine, p%ParamGlobal,  p%ParamTurbine, p%NumSC2CtrlGlob, y%fromSCglob, p%NumSC2Ctrl, y%fromSC, errStat, errMsg )
      ! TODO: Check errors    
#else
         ...
         !call DLL_SC_Subroutine ( p%nTurbines, p%nInpGlobal, p%NumCtrl2SC, p%NumParamGlobal, ParamGlobal, p%NumParamTurbine, ParamTurbine, p%NumStatesGlobal, p%NumStatesTurbine, p%NumSC2CtrlGlob, p%NumSC2Ctrl, errStat, errMsg ) 
         call DLL_SC_GetInitData_Subroutine ( p%nTurbines, p%NumParamGlobal, p%NumParamTurbine, p%ParamGlobal,  p%ParamTurbine, p%NumSC2CtrlGlob, y%fromSCglob, p%NumSC2Ctrl, y%fromSC, errStat, errMsg ) 
         ! TODO: Check errors
