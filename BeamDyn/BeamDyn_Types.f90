...
! =========  BD_OtherStateType  =======
  TYPE, PUBLIC :: BD_OtherStateType
    TYPE(BD_ContinuousStateType) , DIMENSION(:), ALLOCATABLE  :: xdot 
    INTEGER(IntKi)  :: n 
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: uuNf 
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: RootForce 
...
! =========  BD_ParameterType  =======
  TYPE, PUBLIC :: BD_ParameterType
    ...
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: Stif0_N1 
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: member_length 
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: segment_length 
...
IF (ALLOCATED(SrcContStateData%uuNf)) THEN
   ...
   IF (.NOT.ALLOCATED(DstContStateData%uuNf)) THEN 
      ...
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyContState: Error allocating DstContStateData%uuNf.'
         RETURN
      END IF
   END IF
...
IF (ALLOCATED(SrcOtherStateData%xdot)) THEN
   ...
   IF (.NOT.ALLOCATED(DstOtherStateData%xdot)) THEN 
      ...
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyOtherState: Error allocating DstOtherStateData%xdot.'
         RETURN
      END IF
   END IF
   DO i1 = LBOUND(SrcOtherStateData%xdot,1), UBOUND(SrcOtherStateData%xdot,1)
      CALL BD_CopyContState( SrcOtherStateData%xdot(i1), DstOtherStateData%xdot(i1), CtrlCode, ErrStat, ErrMsg )
   ENDDO
ENDIF
...
IF (ALLOCATED(SrcOtherStateData%uuNf)) THEN
   ...
   IF (.NOT.ALLOCATED(DstOtherStateData%uuNf)) THEN 
      ...
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyOtherState: Error allocating DstOtherStateData%uuNf.'
         RETURN
      END IF
   END IF
   DstOtherStateData%uuNf = SrcOtherStateData%uuNf
ENDIF
IF (ALLOCATED(SrcOtherStateData%RootForce)) THEN
   i1_l = LBOUND(SrcOtherStateData%RootForce,1)
   i1_u = UBOUND(SrcOtherStateData%RootForce,1)
   IF (.NOT.ALLOCATED(DstOtherStateData%RootForce)) THEN 
      ALLOCATE(DstOtherStateData%RootForce(i1_l:i1_u),STAT=ErrStat)
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyOtherState: Error allocating DstOtherStateData%RootForce.'
         RETURN
      END IF
   END IF
   DstOtherStateData%RootForce = SrcOtherStateData%RootForce
ENDIF
...
DO i1 = LBOUND(OtherStateData%xdot,1), UBOUND(OtherStateData%xdot,1)
  CALL BD_DestroyContState( OtherStateData%xdot(i1), ErrStat, ErrMsg )
ENDDO
...
ENDIF
IF (ALLOCATED(OtherStateData%RootForce)) THEN
   DEALLOCATE(OtherStateData%RootForce)
ENDIF
...
DO i1 = LBOUND(InData%xdot,1), UBOUND(InData%xdot,1)
  CALL BD_PackContState( Re_xdot_Buf, Db_xdot_Buf, Int_xdot_Buf, InData%xdot(i1), ErrStat, ErrMsg, .TRUE. ) ! xdot 
...
ENDDO
  ...
  Re_BufSz    = Re_BufSz    + SIZE( InData%uuNf )  ! uuNf 
  Re_BufSz    = Re_BufSz    + SIZE( InData%RootForce )  ! RootForce 
  IF ( Re_BufSz  .GT. 0 ) ALLOCATE( ReKiBuf(  Re_BufSz  ) )
  IF ( Db_BufSz  .GT. 0 ) ALLOCATE( DbKiBuf(  Db_BufSz  ) )
  IF ( Int_BufSz .GT. 0 ) ALLOCATE( IntKiBuf( Int_BufSz ) )
DO i1 = LBOUND(InData%xdot,1), UBOUND(InData%xdot,1)
  CALL BD_PackContState( Re_xdot_Buf, Db_xdot_Buf, Int_xdot_Buf, InData%xdot(i1), ErrStat, ErrMsg, OnlySize ) ! xdot 
...
ENDDO
  ...
  IF ( ALLOCATED(InData%uuNf) ) THEN
    ...
    Re_Xferred   = Re_Xferred   + SIZE(InData%uuNf)
  ENDIF
  IF ( ALLOCATED(InData%RootForce) ) THEN
    IF ( .NOT. OnlySize ) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%RootForce))-1 ) =  PACK(InData%RootForce ,.TRUE.)
    Re_Xferred   = Re_Xferred   + SIZE(InData%RootForce)
  ENDIF
...
DO i1 = LBOUND(OutData%xdot,1), UBOUND(OutData%xdot,1)
 ! first call BD_PackContState to get correctly sized buffers for unpacking
  CALL BD_PackContState( Re_xdot_Buf, Db_xdot_Buf, Int_xdot_Buf, OutData%xdot(i1), ErrStat, ErrMsg, .TRUE. ) ! xdot 
  IF(ALLOCATED(Re_xdot_Buf)) THEN
  ...
  IF(ALLOCATED(Int_xdot_Buf)) THEN
    ...
    Int_Xferred = Int_Xferred + SIZE(Int_xdot_Buf)
  ENDIF
  CALL BD_UnPackContState( Re_xdot_Buf, Db_xdot_Buf, Int_xdot_Buf, OutData%xdot(i1), ErrStat, ErrMsg ) ! xdot 
ENDDO
  ...
  DEALLOCATE(mask1)
    Re_Xferred   = Re_Xferred   + SIZE(OutData%uuNf)
  ENDIF
  IF ( ALLOCATED(OutData%RootForce) ) THEN
  ALLOCATE(mask1(SIZE(OutData%RootForce,1))); mask1 = .TRUE.
    OutData%RootForce = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%RootForce))-1 ),mask1,OutData%RootForce)
  DEALLOCATE(mask1)
    Re_Xferred   = Re_Xferred   + SIZE(OutData%RootForce)
  ENDIF
  Re_Xferred   = Re_Xferred-1
  Db_Xferred   = Db_Xferred-1
  Int_Xferred  = Int_Xferred-1
...
ENDIF
IF (ALLOCATED(SrcParamData%Stif0_N1)) THEN
   i1_l = LBOUND(SrcParamData%Stif0_N1,1)
   i1_u = UBOUND(SrcParamData%Stif0_N1,1)
   i2_l = LBOUND(SrcParamData%Stif0_N1,2)
   i2_u = UBOUND(SrcParamData%Stif0_N1,2)
   IF (.NOT.ALLOCATED(DstParamData%Stif0_N1)) THEN 
      ALLOCATE(DstParamData%Stif0_N1(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat)
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyParam: Error allocating DstParamData%Stif0_N1.'
         RETURN
      END IF
   END IF
   DstParamData%Stif0_N1 = SrcParamData%Stif0_N1
ENDIF
...
IF (ALLOCATED(SrcParamData%segment_length)) THEN
   ...
   IF (.NOT.ALLOCATED(DstParamData%segment_length)) THEN 
      ALLOCATE(DstParamData%segment_length(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat)
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyParam: Error allocating DstParamData%segment_length.'
         RETURN
      END IF
   END IF
...
ENDIF
   ...
   DstParamData%piter = SrcParamData%piter
...
IF (ALLOCATED(SrcParamData%bc)) THEN
   ...
   IF (.NOT.ALLOCATED(DstParamData%bc)) THEN 
      ...
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyParam: Error allocating DstParamData%bc.'
         RETURN
      END IF
   END IF
...
IF (ALLOCATED(SrcParamData%F_ext)) THEN
   ...
   IF (.NOT.ALLOCATED(DstParamData%F_ext)) THEN 
      ...
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_CopyParam: Error allocating DstParamData%F_ext.'
         RETURN
      END IF
   END IF
...
ENDIF
IF (ALLOCATED(ParamData%Stif0_N1)) THEN
   DEALLOCATE(ParamData%Stif0_N1)
ENDIF
...
ENDIF
 ...
 ! buffers to store meshes, if any
  ...
  Re_BufSz    = Re_BufSz    + SIZE( InData%Stif0_N1 )  ! Stif0_N1 
  ...
  ENDIF
  IF ( ALLOCATED(InData%Stif0_N1) ) THEN
    IF ( .NOT. OnlySize ) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%Stif0_N1))-1 ) =  PACK(InData%Stif0_N1 ,.TRUE.)
    Re_Xferred   = Re_Xferred   + SIZE(InData%Stif0_N1)
  ENDIF
  ...
  IF ( ALLOCATED(InData%segment_length) ) THEN
    IF ( .NOT. OnlySize ) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%segment_length))-1 ) =  PACK(InData%segment_length ,.TRUE.)
  ...
  Int_Xferred   = Int_Xferred   + 1
  IF ( .NOT. OnlySize ) IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = (InData%piter )
  Int_Xferred   = Int_Xferred   + 1
 ...
 ! buffers to store meshes, if any
  ...
  ENDIF
  IF ( ALLOCATED(OutData%Stif0_N1) ) THEN
  ALLOCATE(mask2(SIZE(OutData%Stif0_N1,1),SIZE(OutData%Stif0_N1,2))); mask2 = .TRUE.
    OutData%Stif0_N1 = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%Stif0_N1))-1 ),mask2,OutData%Stif0_N1)
  DEALLOCATE(mask2)
    Re_Xferred   = Re_Xferred   + SIZE(OutData%Stif0_N1)
  ENDIF
  ...
  ENDIF
  IF ( ALLOCATED(OutData%segment_length) ) THEN
  ALLOCATE(mask2(SIZE(OutData%segment_length,1),SIZE(OutData%segment_length,2))); mask2 = .TRUE.
    OutData%segment_length = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%segment_length))-1 ),mask2,OutData%segment_length)
  DEALLOCATE(mask2)
  ...
  Int_Xferred   = Int_Xferred   + 1
  OutData%piter = IntKiBuf ( Int_Xferred )
  Int_Xferred   = Int_Xferred   + 1
...
IF (ALLOCATED(SrcinputfileData%kp_member)) THEN
   ...
   IF (.NOT.ALLOCATED(DstinputfileData%kp_member)) THEN 
      ALLOCATE(DstinputfileData%kp_member(i1_l:i1_u),STAT=ErrStat)
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'BD_Copyinputfile: Error allocating DstinputfileData%kp_member.'
         RETURN
      END IF
   END IF
...
ENDIF
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred   = Int_Xferred   + 1
  IF ( .NOT. OnlySize ) IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = (InData%kp_total )
  Int_Xferred   = Int_Xferred   + 1
  IF ( ALLOCATED(InData%kp_member) ) THEN
    IF ( .NOT. OnlySize ) IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(InData%kp_member))-1 ) = PACK(InData%kp_member ,.TRUE.)
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred   = Int_Xferred   + 1
  IF ( ALLOCATED(OutData%kp_member) ) THEN
  ALLOCATE(mask1(SIZE(OutData%kp_member,1))); mask1 = .TRUE.
    OutData%kp_member = UNPACK(IntKiBuf( Int_Xferred:Re_Xferred+(SIZE(OutData%kp_member))-1 ),mask1,OutData%kp_member)
  DEALLOCATE(mask1)
