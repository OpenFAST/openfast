...
The last example is to examine different time integrators that had been implemented in BeamDyn. The NREL 5MW blade was used as a cantilever while a white noise force generated by HydroDyn is applied at the free tip along the edge direction. The damping coefficient is set to $1.6 \times 10^{-2}$. Figure~\ref{E4Force} showed the time history of the applied force and its power spectral density (PSD). The edge direction responses are plotted in Figure~\ref{E4Displacement}. Four cases were conducted in BeamDyn: the first three cases were using AM2 time integrator with time step size: $5 \times 10^{-2}$ s, $5 \times 10^{-3}$ s, and $5 \times 10^{-2}$ s, respectively, while the fourth case was using RK4 time integrator with time step size $5 \times 10^{-6}$.  It can be observed that {\color{red} discussion needed}.
\begin{figure}
    \centering
    \begin{tabular}{c}
...
\end{tabular}
\caption{Applied white noise force at the tip of a cantilevered NREL 5MW blade.}
\label{E4Force}
\end{figure} 
\begin{figure}
    \centering
...
\end{tabular}
\caption{Edge direction responses of a cantilevered NREL 5MW blade under white noise excitation.}
